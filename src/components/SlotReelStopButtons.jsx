...
/**
 * 筐体上・各リール窓の下に STOP ボタンを配置（Phase A 手動停止）
 */
export default function SlotReelStopButtons({
  ...
  if (!visible || spectatorMode) return null;
      ...
      {[0, 1, 2].map((reelIdx) => {
        ...
        return (
          <button
            ...
            className={`pointer-events-auto absolute flex items-center justify-center rounded-md border font-black uppercase tracking-wide transition-all ${
              stopped
            ...
            } disabled:cursor-not-allowed disabled:opacity-35 text-[9px] sm:text-[10px]`}
            style={{
